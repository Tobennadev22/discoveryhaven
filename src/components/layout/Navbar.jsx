      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <Link to="/" className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="font-cherry text-xl text-slate-900 hidden sm:block">
              Discovery Haven
            </span>
          </Link>
              ...
              <Link
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold font-body transition-colors ${
                  location.pathname === link.href
                    ...
                    : "text-slate-900 hover:text-aqua hover:bg-aqua/5"
                }`}
              >
                {link.label}
              </Link>
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 px-4 py-6 space-y-2">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              to={link.href}
              className="block px-4 py-3 rounded-xl font-bold font-body text-slate-900 hover:bg-aqua/5 hover:text-aqua transition-colors"
          ...
          <div className="pt-4">
            <Button
              ...
              onClick={() => {
                setOpen(false);
                window.location.href = "/haven-academy";
              }}
            >
              Enrol Your Child
            </Button>
